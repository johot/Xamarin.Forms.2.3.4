...
namespace Xamarin.Forms.Controls.TestCasesPages
{
    ...
    public class NavigationStackTests : ContentPage
    {
        ...
        public NavigationStackTests()
        {
            ...
            pushBtn.Clicked +=
                async (sender, args) => await ((Button)sender).Navigation.PushAsync(new NavigationStackTests());

            pushModalBtn.Clicked +=
                async (sender, args) =>
                    await ((Button)sender).Navigation.PushModalAsync(new NavigationPage(new NavigationStackTests()));

            pushNoAnimBtn.Clicked +=
                async (sender, args) => await ((Button)sender).Navigation.PushAsync(new NavigationStackTests(), false);

            pushModalNoAnimBtn.Clicked +=
                async (sender, args) =>
                    await ((Button)sender).Navigation.PushModalAsync(new NavigationPage(new NavigationStackTests()),
                        false);
            ...
            insertPageBtn.Clicked +=
                (sender, args) => ((Button)sender).Navigation.InsertPageBefore(new NavigationStackTests(), this);
            ...
            removePageBtn.Clicked += (sender, args) =>
            {
                ...
                if (navPage != null)
                {
                    navPage.Navigation.RemovePage(
                        (Page)
                        ((IPageController)navPage).InternalChildren[
                            ((IPageController)navPage).InternalChildren.Count - 2]);
                }
            };
