...
namespace Xamarin.Forms.Controls
{
    public class MasterDetailPageTabletPage : ContentPage
    {
        public MasterDetailPageTabletPage()
        {
            ...
            btn.Clicked +=
                async (sender, e) => await Navigation.PushModalAsync(new Issue1461Page(MasterBehavior.Default, null));

            var btn1 = new Button { Text = "Default (old behavior) but always hide toolbar button" };
            btn1.Clicked +=
                async (sender, e) => await Navigation.PushModalAsync(new Issue1461Page(MasterBehavior.Default, false));

            var btn2 = new Button { Text = "Popover - (always show it as a popover) but  hide toolbar button" };
            btn2.Clicked +=
                async (sender, e) => await Navigation.PushModalAsync(new Issue1461Page(MasterBehavior.Popover, false));
            ...
            btn3.Clicked +=
                async (sender, e) => await Navigation.PushModalAsync(new Issue1461Page(MasterBehavior.Popover, null));

            //we throw an exception here if you try to toggle it 
            var btn4 = new Button
            {
                Text = "Split - (always show it as splitview , toogle master always visible, throws expception)"
            };
            btn4.Clicked +=
                async (sender, e) => await Navigation.PushModalAsync(new Issue1461Page(MasterBehavior.Split, null));

            var btn5 = new Button
            {
                Text = "SplitOnPortrait Portrait - (always show it as splitview in Portrait, throws expception)"
            };
            btn5.Clicked +=
                async (sender, e) =>
            ...
            var btn6 = new Button
            {
                Text = "SplitOnLandscape Landscape - (always show it as splitview in Landscape, throws expception))"
            };
            btn6.Clicked +=
                async (sender, e) =>
            ...
            Content = new StackLayout
            {
                ...
                Children = { btn, btn1, btn2, btn6, btn3, btn4, btn5 },
                Spacing = 20
            };
        }
    }
