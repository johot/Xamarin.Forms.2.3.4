...
namespace Xamarin.Forms.Controls
{
    ...
    internal class MultiBindingHack : INotifyPropertyChanged
    {
        ...
        {
            LabelWithBounds = string.Format("{{X={0:0.00} Y={1:0.00} Width={2:0.00} Height={3:0.00}}}", element.X,
                element.Y, element.Width, element.Height);
            ...
            element.PropertyChanged += (sender, args) =>
            {
                ...
                {
                    LabelWithBounds = string.Format("{{X={0:0.00} Y={1:0.00} Width={2:0.00} Height={3:0.00}}}",
                        element.X, element.Y, element.Width, element.Height); // super hack
                }
            };
        }
