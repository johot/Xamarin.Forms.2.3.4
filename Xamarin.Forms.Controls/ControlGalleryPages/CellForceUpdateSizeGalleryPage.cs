						...
						Device.OnPlatform(
							iOS: () =>
							{
								if (typeof(T) == typeof(MyViewCell))
									target.Text = "I am an exceptionally long string that should cause the label to wrap, thus increasing the size of the cell such that the entirety of the string is readable by human eyes. Hurrah. Odd, though, that not every label is fully rendered until I scroll.";
