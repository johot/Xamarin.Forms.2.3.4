...
namespace Xamarin.Forms.Platform.MacOS
{
	public class EventTracker : IDisposable
	{
		...
		readonly Dictionary<IGestureRecognizer, NSGestureRecognizer> _gestureRecognizers =
			new Dictionary<IGestureRecognizer, NSGestureRecognizer>();
		...
		protected virtual NSGestureRecognizer GetNativeRecognizer(IGestureRecognizer recognizer)
		{
			...
			NSGestureRecognizer uiRecognizer;

			var tapRecognizer = recognizer as TapGestureRecognizer;
			if (tapRecognizer != null)
			{
				uiRecognizer = CreateTapRecognizer(tapRecognizer.NumberOfTapsRequired, r =>
				{
			...
			if (pinchRecognizer != null)
			{
				...
				uiRecognizer = CreatePinchRecognizer(r =>
				{
					...
					originPoint = NSApplication.SharedApplication.KeyWindow.ContentView.ConvertPointToView(originPoint,
						eventTracker._renderer.NativeView);
					...
					switch (r.State)
					{
						...
						case NSGestureRecognizerState.Ended:
							...
							break;
						case NSGestureRecognizerState.Possible:
							break;
						default:
			...
			uiRecognizer = CreatePanRecognizer(r =>
			{
				var eventTracker = weakEventTracker.Target as EventTracker;
				var view = eventTracker?._renderer?.Element as View;
		...
		void ModelGestureRecognizersOnCollectionChanged(object sender,
		...
		void OnElementChanged(object sender, VisualElementChangedEventArgs e)
		{
			...
			if (e.NewElement == null || ElementGestureRecognizers == null)
				return;
			// hook
			ElementGestureRecognizers.CollectionChanged += _collectionChangedHandler;
			LoadRecognizers();
